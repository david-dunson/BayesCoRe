...
We now study the properties of the extrinsic prior and posterior. One important task is to quantify the difference between extrinsic and intrinsic ones. Due to similar reasoning for prior and posterior, we use some general notation. Let $\pi_{\mc R}(\theta)$ be the normalized density in $\mc R$ such that $\int \pi_{\mc R}(\theta)=1$, which is $\pi_{0,\mc R}(\theta)$ when studying prior and $L(y;\theta)\pi_{0,\mc R}(\theta)$ when studying posterior; $\Pi(.)$ and $\tilde\Pi(.)$ to represent the measures under intrinsic and extrisic methods associated with either prior or posterior distribution. 

Suppose $\Phi:\mathbb R^n \rightarrow \mathbb R^m$ with $n>m$ is Lipschitz, then the co-area formula \citep{diaconis2013manifold} is,
	\begin{equation}
\ \int_{\mathbb{R}^n}  f(\theta)J_N\Phi(\theta)\lambda^n(d \theta)
	=\ \int_{\mathbb{R}^m}  \int_{\Phi^{-1}(y)}f(\theta) \mc H^{n-m}(d\theta)\lambda^m(d y),
	\end{equation}
where $\mathcal{H}^{k}(d \theta)$ denotes a $k$-dimensional Hausdorff measure, $\lambda^k(d\theta)$ a $k$-dimensional Lebesgue measure, $J_N(.)$ is the Jacobian. Having $\Phi(\theta)=v(\theta)$ yields $J_N v(\theta)=\|\nabla v(\theta)\|$. Having $f(\theta)=\frac{1}{\|\nabla v(\theta)\|}$ and $f(\theta)=\frac{g(\theta)}{\|\nabla v(\theta)\|}$ yield the marginal density and conditional expectation:
\begin{equation}
	\begin{aligned}
m(y)= & \int_{v^{-1}(y)} \frac{ 1 }{ \| \nabla v(\theta) \| } \mathcal{H}^{n-1}(d \theta),\\
\mathbb{E}(g(\theta) \mid y)= & \int_{v^{-1}(y)} \frac{ g(\theta) }{ \| \nabla v(\theta) \| } \mathcal{H}^{n-1}(d \theta).
		\end{aligned}
\end{equation}

We obtain results on the 1-Wasserstein distance $W_1(\Pi,\tilde\Pi)$ between the extrinsic and intrinsic distributions. 

\begin{remark}
Let $W_1(\Pi,\tilde\Pi)=\underset{\gamma\in \Gamma(\Pi,\tilde\Pi)}{\inf}\int \|(x-y\| d\gamma(x,y)$ be the 1-Wasserstein distance between the extrinsic and intrinsic distributions, then
$$ \underset{\lambda \rightarrow 0}\lim W_1(\Pi,\tilde\Pi)=0.$$
Further, if $\mathbb E (\|\theta\|)<\infty$ with respect to $\pi_{\mc R}(\theta)$, and if there is a $t$-ball surrounding $\mc D$ with bounded norm and marginal density, $\{\theta: v(\theta)<t , \mathbb{E}(\|\theta\| \mid t^{*})<\infty, m(t^{*})<\infty$\},
\begin{equation}
W_1(\Pi,\tilde\Pi)\le \lambda  \left[ \mathbb{E}(\|\theta\| \mid 0) \underset{t^*\in (0,t)}\sup  m(t^{*}) + \underset{t^*\in (0,t)}\sup  \mathbb E (\|\theta\|\mid t^*) \right] + \exp(- \lambda^{-1} t ) (1+\mathbb E (\|\theta\|)) 
\end{equation}
\end{remark}
...
Using co-area formula, taking 
 $f(\theta)=\frac{\exp(-\lambda^{-1} v(\theta))}{J_N v(\theta)}=\frac{\exp(-\lambda^{-1} v(\theta))}{\|\nabla v(\theta)\|}$ yields
	\begin{equation}
	\begin{aligned}
	Z_\lambda
	 = \ \int_{\mathbb{R}^n}  \exp(- \lambda^{-1} v(\theta)) \pi_{\mc R}(\theta) d \theta
	& = \int_\mathbb{R}  \left[ \int_{v^{-1}(y)} \frac{ \exp(- \lambda^{-1} v(\theta) ) }{ \| \nabla v(\theta) \| } \mathcal{H}^{n-1}(d \theta) \right] \mathbbm{1}_{y \ge 0}  d y \\
	& = \int_\mathbb{R}  \left[ \int_{v^{-1}(y)} \frac{ 1 }{ \| \nabla v(\theta) \| } \mathcal{H}^{n-1}(d \theta) \right] \exp(- \lambda^{-1} y ) \mathbbm{1}_{y \ge 0}  d y .
		\end{aligned}
	\end{equation}
	 Taking $f(\theta)=\frac{\mathbbm{1}_{v(\theta)=0}}{\|\nabla v(\theta)\|}$ yields 
	\begin{equation}
	Z_0
	= \int_\mathbb{R} \mathbbm{1}_{v(\theta)=0} \pi_{\mc R}(\theta) d \theta
	= \int_\mathbb{R} \left[ \int_{v^{-1}(y)} \frac{ 1 }{ \| \nabla v(\theta) \| } \mathcal{H}^{d-1}(d \theta) \right]\mathbbm{1}_{y=0} dy   =  \int_{v^{-1}(0)} \frac{ 1 }{ \| \nabla v(\theta) \| } \mathcal{H}^{d-1}(d \theta) 
	\end{equation}

Clearly $Z_\lambda \ge Z_0$. Letting $\mathbb{E}(g(\theta) \mid y)= \int_{v^{-1}(y)} \frac{ g(\theta) }{ \| \nabla v(\theta) \| } \mathcal{H}^{n-1}(d \theta)$ and $m(y)= \int_{v^{-1}(y)} \frac{ 1 }{ \| \nabla v(\theta) \| } \mathcal{H}^{n-1}(d \theta)$,

	\begin{equation}
		\begin{aligned}
	&\int_{\mathbb{R}^n} g(\theta)  \left[ \frac{ \exp(- \lambda^{-1} v(\theta)) } {  Z_\lambda}  - 
	 \frac{ \mathbbm{1}_{v(\theta)=0} } {  Z_0} 
	\right]  \pi_{\mc R}(\theta) d \theta \\
	&= \int_\mathbb{R}  \mathbb{E}(g(\theta) \mid y)  \left[ \frac{ \exp(- \lambda^{-1} y) \mathbbm{1}_{y \ge 0}} {  Z_\lambda}  - 
	 \frac{ \mathbbm{1}_{y=0} } {  Z_0} 
	\right] d y \\  
	&=	\int_\mathbb{R}  \mathbb{E}(g(\theta) \mid y)  \left[ \frac{  1} {  Z_\lambda}  - 
	 \frac{ 1 } {  Z_0} 
	\right]\mathbbm{1}_{y=0} d y  + \int_\mathbb{R}  \mathbb{E}(g(\theta) \mid y)\frac{ \exp(- \lambda^{-1} y)} {  Z_\lambda}  
	  \mathbbm{1}_{y > 0} d y \\
	& \le \mathbb{E}(\|g(\theta)\| \mid 0) \left[ \frac{ 1 } {  Z_0} -\frac{  1} {  Z_\lambda}   
	 \right] + \int_\mathbb{R}  \mathbb{E}(\|g(\theta)\| \mid y) \frac{ \exp(- \lambda^{-1} y)} {  Z_\lambda}  
	\mathbbm{1}_{y > 0} d y \\
	& \le \mathbb{E}(\|g(\theta)\| \mid 0)  \frac{   \int_\mathbb{R}  m(y) \exp(- \lambda^{-1} y ) \mathbbm{1}_{y > 0}  d y } {  Z_0^2}   
	   + \int_\mathbb{R}  \mathbb{E}(\|g(\theta) \|\mid y) \frac{ \exp(- \lambda^{-1} y)} {  Z_0}  
	\mathbbm{1}_{y > 0} d y \\
		\end{aligned}
	\end{equation}
	
	Note $\int_\mathbb{R} m(y) \mathbbm{1}_{y \ge 0}  dy =\int_\mathbb{R} \pi_{\mc R}(\theta) =1$,
	
	\begin{equation}
	\label{wass_part1}
	\begin{aligned}
	\int_\mathbb{R}  m(y)\exp(- \lambda^{-1} y ) \mathbbm{1}_{y > 0}   dy 
	&= \int_0^{t}  m(y) \exp(- \lambda^{-1} y ) dy + \int_t^{\infty}  m(y) \exp(- \lambda^{-1} y ) dy \\
	&\le \underset{t^*\in (0,t)}\sup m(t^{*})\int_0^{t} \exp(- \lambda^{-1} y ) dy + \exp(- \lambda^{-1} t )\int_t^{\infty}  m(y) dy \\
	&\le  \lambda \underset{t^*\in (0,t)}\sup  m(t^{*})  + \exp(- \lambda^{-1} t )
	\end{aligned}
\end{equation}

Since $\|g(\theta)\|\le \|\theta\|$ and $\int_\mathbb{R} \mathbb{E}(\|\theta \|\mid y) \mathbbm{1}_{y > 0}  dy \le \int_\mathbb{R} \mathbb{E}(\|\theta \|\mid y) \mathbbm{1}_{y \ge 0}  dy =\int_\mathbb{R} \|\theta\|\pi_{\mc R}(\theta) d\theta= \mathbb E (\|\theta\|)$,

	\begin{equation}
		\label{wass_part2}
	\begin{aligned}
	\int_\mathbb{R}  \mathbb{E}(\| g(\theta)\| \mid y){ \exp(- \lambda^{-1} y)} 
	\mathbbm{1}_{y > 0} d y
	& \le  \int_\mathbb{R}  \mathbb{E}(\| \theta\| \mid y){ \exp(- \lambda^{-1} y)} 
	\mathbbm{1}_{y > 0} d y\\
	&\le \underset{t^*\in (0,t)}\sup \mathbb{E}(\| \theta\| \mid t^*)\int_0^{t} \exp(- \lambda^{-1} y ) dy + \exp(- \lambda^{-1} t )\int_t^{\infty}   \mathbb{E}(\| \theta\| \mid y) dy \\
	&\le  \lambda \underset{t^*\in (0,t)}\sup  \mathbb E (\|\theta\|\mid t^*) + \exp(- \lambda^{-1} t ) \mathbb E (\|\theta\|)
	\end{aligned}
\end{equation}

Combining \eqref{wass_part1} and \eqref{wass_part2},


\begin{equation}
\begin{aligned}
& \underset{g:\|g\|_L\le 1}\sup \int g(x) \Pi(dx) -  \int g(y) \tilde\Pi(dy) 
	 \\  \le &\lambda  \left[ \mathbb{E}(\|\theta)\| \mid 0) \underset{t^*\in (0,t)}\sup  m(t^{*}) + \underset{t^*\in (0,t)}\sup  \mathbb E (\|\theta\|\mid t^*) \right] + \exp(- \lambda^{-1} t ) (1+\mathbb E (\|\theta\|)) \\
\end{aligned}
\end{equation}
