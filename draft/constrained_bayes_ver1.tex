...
We now study the property of the extrinsic prior. One important question is the approximation difference between extrinsic and intrinsic prior. We first focus on the first case in \eqref{exact_prior1}.
...
\end{theorem}
\leo{via definition of total variation and $K(\theta;\mc D)=1$  $\theta\in\mc D$}
...
% We may include some parameter \lambda controlling how close the approximation is & show that the distance between the indicator function and approximation decreases to zero in \lambda or something.  Also the distance between the intrinsic prior and extrinsic prior also decreases to zero in \lambda.  But as \lambda decreases the energy barrier increases and posterior computation becomes more intractable etc etc.
