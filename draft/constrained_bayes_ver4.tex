...
\begin{remark}
	Assuming $\|\nabla v(\theta)\|\neq 0$, then
\begin{equation}
	P(E\mid v(\theta)=\beta)= \frac{1}{m(\beta)}\int_{E} \frac{\pi_{\mc
	R}(\theta) \mathbbm{1}_{v({\theta})=\beta}}{\|\nabla v(\theta)\|}d\theta
\end{equation}
is a valid r.c.p., where the integral is taken with respect to
$(p-d)$-dimensional Lebesgue measure and ${m(\beta)}=\int \frac{\pi_{\mc
	R}(\theta) \mathbbm{1}_{v({\theta})=\beta}}{\|\nabla v(\theta)\|}d\theta
$.
	\end{remark}
	\begin{proof}
