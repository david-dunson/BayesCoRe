...
\eqref{approximatePosterior} becomes exact; using small $\lambda_k>0$, \eqref{approximatePosterior}
 conventional Monte Carlo approach can be carried.
...
\subsubsection{Exact CORE}

The previous strategy is generally applicable for approximating the indicator. In some cases, when the constraint function $v(\theta)$ takes simple form, one can easily project the relaxed parameter $\theta^*\in \mc R$ back into $\mc D$ and use them to reparameterize the exact posterior density. This avoids the approximation error.  Although this is more limited than the previous one, it still covers a large range of useful constraints, such as norm constraint (e.g. $v(\theta)=\|\theta\|_m-1$,  with $m=1$ in simplex modeling, $m=2$ in hypersphere modeling), in which the projection is simply a re-normalization.

Our goal is to use variable transformation to derive a density for $\theta^*\in \mc R$ based on $\pi_{\mc D}(\theta)$, sample $\theta^*$, and use its projection for posterior inference. One thing we need is to make the projection invertiable. Observing the projection is from $\{\theta^*: v(\theta^*)=w\}$ to $\{\theta:v(\theta)=\bf 0 \}$ using $\theta=g(\theta^*;w)$ with $w=v(\theta^*)$, one can augment the density of $w$ to $\theta$, so that $\theta^*=g^{-1}(\theta;w)$, leading to

\be
\label{reparametrizedPosterior}
P(\theta\in A\mid y) = & 
 \int_{\mc W}\int_A  \pi_{\mc D}(\theta\mid y)  \pi(w)  d\theta dw 
\\ 
= & 
\int_{\mc W} \int_{g(\theta^*; w)\in A} \pi(\theta^*,w \mid y ) d\theta^* dw, \qquad
 \pi(\theta^*,w \mid y) = \pi_{\mc D}(g(\theta^*;w)\mid y) J(g(\theta^*;w)) \pi(w)  ,
\ee
for measure over $A\subset\mc D$, where $\mc W$ is the support of $w$, $\int_{\mc W} \pi(w) dw =1$, $J(g(\theta^*;w))$ is the Jacobian. Since $J(g(\theta^*;w))=\frac{1}{J(g^{-1}(\theta;w))}$, it is easy to verify the second line can transform back to first line. Since $w$ is independent of $\theta$, $\pi(w)$ can take any distribution as long as its support agrees with the one of $v(\theta^*)$. This is very similar to the parameter expansion strategy used in augmenting probit regression \citep{liu1999parameter}, except they relax the variance of the latent variable from $1$ to a redundant parameter, and we relax the constraint function $v(\theta)$ from $\bf 0$.

To provide concrete illustration of \eqref{reparametrizedPosterior}, we consider a toy example $(p-1)$-simplex 
$\{\theta:  \sum_{i=1}^p\theta_k=1, \theta_i\in(0,1) \text{ for } i=1,\ldots,p\}$, using Dirichlet distribution $\theta\mid y\in \text{Dir}(\alpha)$
\be
\pi_{\mc D}(\theta \mid y)\propto \prod_{i=1}^p \left(\theta_i^{\alpha-1}\mathbbm{1}_{\theta_i\in(0,1)} \right)\mathbbm{1}_{\sum_{i=1}^p\theta_i=1}.
\ee
Relaxing simplex into space $\mc R=[0,1]^p$, one would have $\sum_{i=1}^p\theta^*_k-1=w$. A viable projection is simply re-scaling $\theta^*$ by  $1/{(w+1)}$. Noticing $\theta_p$ and $\theta_p^*$ are deterministic given $w$ and other $\{\theta_i,\theta^*_i\}_{i=1,\ldots,(p-1)}$, we drop these two. Using \eqref{reparametrizedPosterior} without the integration,
\be
\pi(\theta^*,w\mid y) \propto (1-\frac{\sum_{i=1}^{p-1} \theta^*_i}{w+1})       \prod_{i=1}^{p-1} (\frac{\theta_i^*}{(w+1)})^{\alpha-1}  (w+1)^{-(p-1)}\mathbbm{1}_{(w+1)>\sum_{i=1}^{p-1}\theta_i   } \pi(w)
\ee
where $(w+1)^{-(p-1)}$ is $J(g(\theta^*;w))$. Therefore, one can sample $\theta^*_1,\ldots \theta^*_{p-1}$ and $w$ directly for this distribution. This relaxation is particularly useful when the posterior is not conjugate on the simplex, such
as hyper-Dirichlet distribution CITES.
...
When $d<p$, although the
$p$-dimensional Lebesgue measure $\mu^p(\mc D)=0$, it is still possible to define a conditional probability given the event $\theta\in \mc D$ (i.e. ${v(\theta)=\bf 0}$). We utilize the concept of {\it regular
...
We now quantify the approximation error of the exponential approximation \eqref{approximatePosterior} in constraint relaxing. 
...
where $\Gamma(\Pi,\tilde\Pi)$ is the family of all joint measures of with $\Pi$ and $\tilde\Pi$ as the marginals.
...
Conditioning the unconstrained density onto $\mc D$ often disrupts the posterior conjugacy. Thanks to the constraint relaxation via approximation or reparameterization, we can sample the posterior directly inside $\mc R$. One can exploit conventional sampling tools such as slice sampling, adaptive Metropolis-Hastings and Hamiltonian Monte Carlo (HMC). In this section, we focus on HMC for its easiness to use and good performance in block sampling with relatively high dimension.

\subsection{Hamiltonian Monte Carlo under Constraint Relaxation}

We provide a brief overview of HMC for continuous $\theta^*$ under constraint relaxation. Discrete extension is possible via recent work of \cite{nishimura2017discontinuous}. For easy notation, we use $q$ to represent $\theta^*$ under approximation \eqref{approximatePosterior}, and $\{\theta^*, w\}$ under reparameterization \eqref{reparametrizedPosterior}.
...
\end{aligned}
\end{equation}
with $\pi(q)$ is the unnormalized density in \eqref{approximatePosterior} or \eqref{reparametrizedPosterior}. %$L(\theta^*;y)\pi_{0,\mc R}(\theta^*) \exp( - \sum_k\frac{|v_k(\theta^*)|^\alpha}{\lambda_k}) / J(v(\theta^*))$ based on \eqref{approximatePosterior} and $f(q)= L(g(\theta^*;w);y)\pi_{0,\mc R}(g(\theta^*;w))J(g(\theta^*;w)) / J(v(g(\theta^*;w)))$ based on \eqref{reparametrizedPosterior}. 
...
R$, it is useful to study the effect of space expansion on the computing efficiency. In this section, we provide some heuristic quantification of the effects and provide a practical guidance on choosing $\pi(w)$ or $\lambda$ in the two constraint relaxation strategies.
...
For approximation \eqref{approximatePosterior}, to minimize approximation
error, one can choose to relax a subset of constraints. Observing  $\mc D=\cap D_k$,
each approximation $\exp(-\frac{|v_k(\theta^*)|^\alpha}{\lambda_k})$ corresponds to a constrained space $\mc D_k$.  The practical strategy is that, for $\mc D_k$'s with $\eta(\theta;\mc D_k)\approx 0$, one use moderate $\lambda_k$ to induce some support expansion (denoted by $\lambda_k\ge \zeta$ with $\zeta$ moderately small but not very close to $0$);  for $\mc D_k$'s without this issue, one uses very small $\lambda_k\approx 0$ to almost always uphold the constraint.
The latter was also suggested by \cite{neal2011mcmc} as creating a high `energy wall'. To reduce the inaccuracy of HMC near the boundary with $\lambda_k\approx 0$, we use random step size $\epsilon$ at each iteration.


The Hessian $\hess_U(q)$ under this strategy is given by
\begin{equation}
...
On the other hand, since reparameterization  \eqref{reparametrizedPosterior} does not involve such trade-off, it is more preferrable despite its less
general application. Letting $\mc Q_\theta\subset \mc D$ denote the support for the constrained $\theta\in \mc D$, the reparameterization changes the support to $Q_{\theta^*}=\{g^{-1}(\theta;w):\theta\in \mc Q\}$. Therefore, one could choose $\pi(w)$ to substantially increase $\eta(\theta^*; Q_{\theta^*})$, provided it does not significantly reduce $\xi^{-1/2}_1(\theta^*)$. In literature related to parameter expansion Gibbs sampling \citep{liu1999parameter}, it was suggested to use flat improper prior for $\pi(w)$; instead, since HMC
is used, we suggest assigning $\pi(w)$ loosely centered at $\bf 0$ (corresponding to $\theta^*=\theta$), which makes $\theta^*$ a mild relaxation of $\theta$
and ensures no substantial change in $\xi^{-1/2}_1(\theta^*)$. More will be illustrated in the next section.

\section{Simulations}

In this section, we run simulations on several toy examples, mainly to compare with existing approaches on computing efficiency and provide empirical evidence
supporting our previous result.

\subsection{Gaussian  under Linear Inequality}
We first consider linear models under linear inequality constraints. Although
recent work proposed a new class of prior with posterior conjugacy \citep{danaher2012minkowski} , one can simply use consider Gaussian prior under our framework. To illustrate
the computing performance, we sample a bivariate Gaussian $\theta \sim \No \left( \mu, I\sigma^2\right)$ subject to linear inequality $\theta\in(0,1)^2,\theta_1+\theta_2<1$,
which is a triangle. In two separate settings, choosing $(\mu, \sigma^2)$ as $([0.3,0.3],1/{10})$  induces a wide-spread distribution centered in the interior of $\mc D$, while $([0.7,0.3]',1/10^4)$  induces a  distribution concentrated on the boundary of $\mc D$. For constraint relaxation, we use the approximation form \eqref{approximatePosterior} with $\exp(-\frac{|v(\theta)|}{\lambda})$, with $v(\theta)=|\theta_1+\theta_2-1|_+ + |-\theta_1|_+ + |-\theta_2|_ + + |\theta_1-1|_+ + |\theta_2-1|_+$.  Since the triangle has $\eta(\theta;\mc D)$ away from $0$, small $\lambda=10^{-8}$
...
To illustrate equality constraint relxation, we generate a simple von Mises--Fisher distribution $\pi_{\mc D}(\theta) \propto \exp(F'\theta)$ on a unit circle $\{(\theta_1,\theta_2):\theta_1^2+\theta_2^2=1\}$. We use $F=(5,5)$ to induce a relatively spread-out  $\theta$ on the manifold.
For sampling,
we compare three strategies: approximate  constraint relaxation using $\exp(-\frac{|\theta'\theta -1|}{\lambda})$ for the indicator, reparameterization   constraint relaxation
using $\theta_1 = \frac{\theta_1^*}{w+1}, w+1= \sqrt{(\theta_1^*)^2+ (\theta_2^*)^2}$ and $\pi(w)\sim \No(0,1)$ and  exact  von Mises--Fisher obtained using `movMF' package.

Unlike the previous linear inequality constraint, the unit circle has $\eta(\theta;\mc D)=0$ for all $\theta\in \mc D$, therefore, some support expansion is needed
for HMC. We test $\lambda = 10^{-3}$, $10^{-4}$ and $10^{-5}$ for approximation
constraint relaxation. To compare the efficiency of HMC, we fix the number of leap-frog steps to $20$ within one iteration HMC, and let software STAN
automatically tune for stable step size. Table~\ref{table_circle} shows the effective sample size per $1000$ iterations, the effective `violation' $|v(\theta)|=|\theta_1+\theta_2-1|$ and the 1-Wasserstein distance $W_1$ as the approximation error. As $W_1$ is numerically computed, to provide a baseline error, we also calculate the average $W_1$ comparing two independent samples from the same exact distribution. The approximation error $W_1$ based on $\lambda= 10^{-5}$ approximation is indistinguishable from this low numerical error, while the other approximations  have slightly larger error but more effective samples.
   ...
   \hline     
    & \multicolumn{4}{c|}{Constraint Relaxation HMC}     & Exact  \\   
       \hline     
        & \multicolumn{3}{c|}{Approximation}     & Reparameterization  \\           
       \hline           
...
To illustrate, we consider $p=3$ and various values of $\alpha\in \{1,0.5,0.1,0.01\}$. We test the performance of constraint relaxation under approximation
and reparameterization. To compare, we also test the standard HMC using coordinate
...
For all HMCs, we fix the number of steps in each iteration to be $30$ and
tune the step size.


Table~\ref{simplex_tb} lists the effective sample sizes under different $\alpha$'s.
As $\alpha$ becomes smaller than $1$, approximation HMC based on constraint relaxation and geodesic HMC become worse in performance, while reparameterizations
based on constraint relaxation or coordinate system are much less impacted.
Figure~\ref{simplex}(b) shows at $\alpha=0.01$, the approximate HMC and geodesic
HMC are stuck for a long time, while reparameterizations
based on constraint relaxation works substantially better. As a well-tested reparameterization, it is not surprising that HMC based on coordinate system still works acceptably well in this case.

The difficulty that approximate HMC based on constraint relxation encountered is well anticipated. \cite{byrne2013geodesic} have previously reported similar
slow-down of geodesic HMC computing on hyper-Dirichlet  distribution CITES with $\alpha<1$.
And our approximate HMC can also be viewed as an  approximation
to the exact geodesic HMC, with the metric tensor product replaced by our exponential function.
...
compares the traceplots of 4 different types of HMCs, which are based on: approximation using constraint
relaxation with $\lambda=10^{-3}$, reparameterization using constraint
relaxation, geodesic flow on simplex \citep{byrne2013geodesic} and coordinate
system.}
\label{simplex}
\end{figure}
   ...
   \hline     
    & \multicolumn{3}{c|}{Constraint Relaxation HMC}     & Geodesic HMC  & Coordinate HMC \\   
        & {Approx $\lambda=10^{-3}$} & {Approx $\lambda=10^{-4}$}      & Reparameterization
        &  \\  \hline         
