...
%Under these constraints, $\pi_\mathcal{D}$ is the pointwise limit of $\tilde{\pi}_\lambda$ for $\mu_\mathcal{R}$ a.e.  $\theta$ in $\mathcal{R}.$

\alex{
When selecting from the broad class of potential constraint functions, care must be taken to ensure the choice is consistent with prior information and with an understanding of the impact on posterior inference.  In our first consideration, where it is assumed that $\theta$ has high probability of residing in a small neighborhood of $\mc D$, the constraint should be chosen to reflect acceptable deviations from $\mc D$. This may take of the form of $\|\nu_\mc D (\theta)\| =\inf_{x\in\mathcal{D}}\sqrt{(x-\theta)^T A (x-\theta)}$. Here, the positive definite matrix $A$ has a singular value decomposition where the smallest singular values are paired with singular vectors oriented in the direction of greatest relaxation.  In the absence of \emph{a priori} information on preferential directions of relaxation, it is more suitable to use \eqref{eq:vfunc} where $k$ may be chosen so that the levels curves of $\|\nu_\mc D \|$ are geometrically similar to the boundary of $\mc D.$

Most importantly, we note that the choice of constraint will inevitably affect posterior inference.  In the each of the example constraints mentioned previously, any directional bias introduced by the choice of $\|\nu_\mc D \| $ disappears in the small $\lambda$ limit. However, the rate at which this bias decays in $\lambda$ depends on the unconstrained posterior, $\mc L(\theta;Y)\pi_{\mc R}(\theta)$, the geometry of $\mc D$, and the choice of constraint.  

With regards to our second motivation, one must also consider the efficiency of posterior sampling when constructing the CORE posterior.  As such, an easy to compute form of $\| \nu_{\mc D}\|$ is desirable. When choosing a constraint function, the minimal conditions that $\| \nu_{\mc D}\|$ is zero on the constrained region and positive otherwise, allow one to balance these competing interests.

One utility of CORE is to approximate
$\bb E[g(\theta) \mid \theta\in\mathcal{D}]$ with the relaxed
...
g(\theta)\pi_\mathcal{D}(\theta|Y) d\mu_\mathcal{R}(\theta) $$
for many suitable choices of $\| \nu_{\mc D}(\theta) \|$ due to the pointwise convergence of $\exp(-\| \nu_{\mc D}\|/\lambda)$ to $\mathbbm{1}_{\mc D}$. }

Rigorous
details for its validity of this approximation, a suitable class of
functions for which it applies and rates of convergence in $\lambda$ are
contained in Section \ref{SEC:Positive_measure_theory}. \alex{For now, we
illustrate a common case of modeling under linear inequality constraints. In this example, we discuss the effects of the choice of $\| \nu_{\mc D}\|$ on estimation of the first moments of $\pi_{\mc D}(\theta|Y)$ when the relaxed CORE density is used to obtain estimates of the first moments of a sharply constrained density.}
...
% \includegraphics[width=1\textwidth, height=2in]{Bivariate_Normal_Triangle_Constraint}
...
\alex{As mentioned previously, the choice for
$\|v_{\mc D}(\theta)\|$ is often not unique. In the initial construction of the relaxed posterior \eqref{EQ:Relaxed_Density_Bivariate_Normal_Triangle}, we chose $\|v_{\mc D}(\theta)\|$ as the minimal Euclidean
distance to the constrained space for an intuitive illustration of the
constraint relaxation. Alternatively, one could have used the minimal $L_1$ distance or a 
constraint that is easier to compute, such as 
\begin{equation}
\|v_{\mc D}(\theta)\| =
\max(\theta_1+\theta_2 -1,0) + \max(-\theta_1,0) + \max(-\theta_2,0),
\label{EQ:Simple_Triangle_Distance}
\end{equation}
which also satisfies $\theta\in\mc D$ if and only if $\|v_{\mc
D}(\theta)\|=0$. 

Figure \ref{FIG:Bivariate_Normal_Triangle_Moments_vs_Constraint} depicts the effects that $\lambda$ and $\| \nu_{\mc D}\|$ have on the first moments of $\tilde{\pi}_\lambda.$  The moment of the sharply constrained posterior, $\pi_{\mc D},$ is $[0.6651, 0.1728]'$.  For large $\lambda$, the CORE density overestimates both components of the mean but does not display much sensitivity to the choice of distance.  However, as $\lambda \to 0$, the moments of $\tilde{\pi}_\lambda$ approach those of $\pi_{\mc D}$ regardless of the choice of constraint function.  In fact, for $\lambda \le 10^{-2}$, the relative error in the approximation of $E[(\theta_1,\theta_2)]$ is less than $3\times 10^{-3}.$ 

% \begin{figure}[H]
% \begin{center}
% \includegraphics[width=1\textwidth]{Moments_vs_Lam_and_Constraints_Normal_Triangle.jpg}
% \end{center}
% \caption{Moments of the CORE posterior under minimal $L_1$ distance, minimal $L_2$ distance, and a simple `distance' choice from \eqref{EQ:Simple_Triangle_Distance}. The sharply constrained distribution is a Gaussian with mean $[0.3,0.3]'$ and covariance $10^{-1}I_2$ truncated over the triangular region $\mc D$. In this case, the mean of the sharply constrained density, $\pi_{\mc D}$, is $[0.6651,0.1728]'$.}
% \label{FIG:Bivariate_Normal_Triangle_Moments_vs_Constraint}
% \end{figure}
...
% \begin{figure}[H]
% \begin{center}
% \includegraphics[width=1\textwidth]{Moments_vs_Lam_and_Constraints_Uniform_Triangle.jpg}
% \end{center}
% \caption{Moments of the CORE posterior under minimal $L_1$ distance, minimal $L_2$ distance, and a simple `distance' choice from \eqref{EQ:Relaxed_Uniform_Triangle_Constraint}. The sharply constrained distribution is uniform over the triangular region $\mc D$. In this case, the mean of the sharply constrained density, $\pi_{\mc D}$, is $[1/3,1/3]'.$}
% \label{FIG:Uniform_Triangle_Moments_vs_Constraint}
% \end{figure}
