...
\end{equation}
where we repress the conditioning on data $Y$ in $\tilde{\pi}_{\lambda}(\theta)$ for concise notation and use $\|v_\mathcal{D}(\theta)\|$ as
a `distance' from $\theta$ to the constrained space.  
We assume $\pi_{\mc R}(\theta)$ is proper  and absolutely continuous with respect to Lesbesgue measure $\mu_\mathcal{R}$ on $\mathcal{R}$. As such, the constraint relaxed posterior $\tilde{\pi}_\lambda(\theta )$ corresponds to a coherent Bayesian probability model. 
...
converges to $\mathbbm{1}_\mathcal{D}(\theta)$ in a pointwise manner, \sout{excluding $\theta \in \partial {\mc D}$ on the boundary of ${\mc D}$.} \aki{The statement about the boundary is 1) irrelevant when ${\mc D}$ has a positive measure and 2) not true if ${\mc D}$ is a manifold of lower dimension --- the boundary of the manifold would be itself relative to the ambient space.} For all $\lambda > 0$, $\tilde{\pi}_\lambda(\theta)$ introduces support outside
...
In the next subsection, we discuss the choice of distance. In the subsequent subsections, we discuss details specific to the cases in which $\mc D$ has positive and zero measure, respectively.
...
%Again, additional technical details on this process will be provided in Section \ref{SEC:Zero_measure_theory}.  
...
%As we discuss in Section \ref{SEC:Zero_measure_theory}, one can view the relaxed posterior as a weighted average of a collection of sharply constrained posteriors over a set of $(r-s)$ dimensional submanifolds (the sets $\nu_{\mc D}^{(-1)}(x))$ in a neighborhood of $\mc D.$ For the distribution over an individual submanifold $\nu_{\mc D}^{(-1)}(x)$, its weight depends both on the distance, $\| v_{\mc D}(\theta)\|$, to $\mc D$ and on the magnitude of the likelihood, $\mathcal{L}(\theta;Y)$, over the set. The second contribution allows for relaxation away from $\mc D$ when justified by the data, $Y$, which would otherwise be unattainable under the sharply constrained posterior. 

%In Section 3.2, in addition to specifying explicit assumptions on the constraint functions, we also present suitable choices for a number of common constraints to further clarify implementation of constraint relaxation in the measure zero setting.  Additionally, the section contains an investigation into the small $\lambda$ behavior of $\tilde{\pi}_\lambda$ by studying the relationship between the relaxed and constrained densities using the co-area formula \cite{federer2014geometric}.  

%First, we present an example for generating samples near a curved torus in $\mathbb{R}^3.$  
...
%Throughout, it is important to note two perspectives for interpreting the results. First, if one wishes to use CORE to approximate sharp constraints, the error bounds contained in the theorems are useful for assessing the accuracy of estimates.  Alternatively, these error bounds can be viewed as proxies for understanding the effects of relaxation resulting from the data.  
...
%For a constrained space with positive measure, recall the minimal condition that the distance function $\|\nu_{\mc D}(\theta)\|$, is zero for $\theta \in \mc D$ and positive otherwise.  As such, CORE is applicable so long as a computationally tractable distance function exists.
...
A short calculation shows that the difference $\bb E[g(\theta)|\theta \in \mc D] - \bb E_{\tilde{\pi}_{\lambda}}[g(\theta)]$ depends on two quantities: the posterior probability of $\mc D$ under the constrained posterior, % $\mc L(\theta;Y)\pi_{\mc R}(\theta),$ 
...
\end{corollary} 
%Secondly, in addition to point estimates, it is possible to approximate probabilities $P(\theta \in \mathcal{F}|\theta \in \mathcal{D})$ and higher moments (e.g. $\bb E[\Pi_j \theta_j^{k_j} |\theta\in\mathcal{D}]$)  so long as these moments exist for the unconstrained posterior density.  These bounds demonstrate that the error in using the relaxed density to approximate $\bb E[g(\theta)|\theta\in\mathcal{D}]$ is proportional to $\sqrt{\lambda} [\int_\mathcal{D}\mathcal{L}(\theta; Y) \pi_\mathcal{R}(\theta)d\mu_\mathcal{R}(\theta)\big]^{-2}$. Specific details of the scaling of $\big|\bb E[g(\theta)|\theta\in\mathcal{D}] - \bb E_{\tilde{\pi}_\lambda}[g(\theta)] \big|$ will depend upon $\mathcal{D}$ and the choice of distance $\|v_{\mc D}(\theta)\|$ so that one should compare estimates obtained for a range of $\lambda$ to assess sensitivity of any posterior inference when using CORE to approximate the sharp constraints.
...
Criteria (a) - (d) may seem restrictive. However, many measure zero constraints can be defined implicitly to satisfy (b) - (d).  In Table \ref{TABLE:Equality_constraints_examples}, we offer a few examples. %\aki{Why arctangent for the constraint function? Its range is $[- \pi / 2, \pi / 2]$. Don't we want $\| v(\theta) \| \to \infty$ as $\theta$ moves away from the constraint?}. 
...
%We provide a brief overview of HMC for continuous $\theta^*$ under constraint relaxation. 
% COMMENT - It seems we focus on continuous parameters throughout so don't need to qualify here.

%Discrete extension is possible via recent work of \cite{nishimura2017discontinuous}.
...
%There are generally two factors limiting the efficiency of HMC: (i) the width of support in constrained space; (ii) the largest eigenvalue of the Hessian matrix. For the former, using $\mc Q$ to denote a support, the width of support is related to the shortest distance to the boundary $\eta (\theta; {\mc Q})= \inf_{\theta'\not\in \mc Q}\|\theta'-\theta\|$. If $\eta (\theta; {\mc Q}) \approx 0$ for all $\theta\in \mc Q$, the proposal would likely be rejected using a large leap-frog step size. In such case, it is useful to utilize CORE to expand support and increase $\eta (\theta; {\mc Q})$ for better computing efficiency. 
...
Fitted AUC                        & 97.9\%                         & 97.1\% & 96.9\% \\ \hline
 ...
 % \\ \hline ESS /1000 Iterations                        & 193.72
 % & 188.10 & 8.15
  \\
  \hline
\end{tabular}
