...
afarray gpu_rand_int(dim4 dim, int k) { return floor(randu(dim) * k); }

afarray gpu_rbern(afarray prob) { return (randu(prob.dims()) < prob).as(u32); }

afarray gpu_pnorm(afarray q) { return 0.5f + erf(q / sqrt(2.0f)) / 2; }

afarray gpu_pnorm_zero(const afarray mu) {
  afarray q = (0.0 - mu);
  return 0.5f + erf(q / sqrt(2.0f)) / 2;
}

afarray gpu_rtruncnorm_zero(afarray mu, afarray posSet, afarray zeroSet) {
  afarray pnorm_0 = gpu_pnorm_zero(mu);

  afarray Z = constant(0.0f, mu.dims());
  afarray u = randu(mu.dims());

  Z(posSet) = (1.0f - pnorm_0(posSet)) * u(posSet) + pnorm_0(posSet);
  Z(zeroSet) = pnorm_0(zeroSet) * u(zeroSet);

  Z(posSet) = qnorm(Z(posSet), mu(posSet));

  Z(zeroSet) = qnorm(Z(zeroSet), mu(zeroSet));
